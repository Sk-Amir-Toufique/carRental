...
const Dashboard = () => {
  ...
  return (
    <div className='px-4 pt-10 md:px-10 flex-1'>
      <Title title="Admin Dashboard" subTitle="Monitor overall platform performane including
      total car , bookings , revenue and recent activities" />
      ...
      my-8 max-w-3xl">
        {dashboardCards.map((card , index) => (
          ...
          rounded-md border border-borderColor'>
            ...
            bg-primary/10">
              <img src={card.icon} className='h-4 w-4'></img>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-start gap-6 mb-8 w-full">
        ...
        <div className="p-4 md:p-6 mb-6 border border-borderColor rounded-md
         md:max-w-xs w-full">
          ...
          <p className="text-3xl mt-6 font-semibold text-primary">{currency}
          {data.monthlyRevenue}</p>
        </div>
      </div>
    </div>
  )
